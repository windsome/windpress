...
export default class Page extends Component {
    ...
    updatePostStatus (id, status) {
        var { updatePost, retrievePosts } = this.props;
        this.setState({modalIsOpen:false, modalMessage:null, modalFunc:null});
        if (id) {
    ...
    render () {
        ...
        var opSetFavor = (evt, id) => {
            evt.preventDefault();
            this.setState({modalIsOpen:true, modalMessage: '确定要放入大厅展示吗？', modalFunc: ()=>this.updatePostFavor(id,1)});
        };
        var opUnsetFavor = (evt, id) => {
            evt.preventDefault();
            this.setState({modalIsOpen:true, modalMessage: '确定要取消大厅展示吗？', modalFunc: ()=>this.updatePostFavor(id,0)});
        };
        var opRefund = (evt, id) => {
            evt.preventDefault();
            this.setState({modalIsOpen:true, modalMessage: '确定要退款展示吗？', modalFunc: ()=>this.updatePostFavor(id,0)});
        };
        ...
        var artworks = postList && postList.map ((id, index)=>{
            ...
            if (isRoot) {
                switch (status) {
                case 0: ops = {  }; break;
                case 1: ops = { opEdit, opDelete, opCheckFail, opPublish }; break;
                case 2: ops = {  }; break;
                case 3: ops = {  }; break;
                case 4: ops = { opRefund }; break;
                case 5: ops = { opRefund }; break;
                case 6: ops = { opRefund }; break;
                case 7: ops = {  }; break;
                default: ops= {}; break;
                }
            }
            if (canPostFavor && status == 2) {
                // only post.status == 2 and user.caps contains POST_FAVOR, can do this.
                ops = { ...ops, opSetFavor, opUnsetFavor };
            }
            artwork.ops = ops;
            return artwork;
        });
